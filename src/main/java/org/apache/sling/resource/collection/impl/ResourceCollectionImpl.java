...
public class ResourceCollectionImpl implements ResourceCollection {
    ...
    public void orderBefore(Resource srcResource, Resource destResource) {
        ...
        if (srcIndex < 0) {
            log.warn(
                    "Collection ordering failed, as there is no resource {} in collection {} for destResource",
                    srcPath,
                    getPath());
            return;
        }
        if (destResource == null) {
            ...
            if (destPath.equals(srcPath)) {
                String message = MessageFormat.format(
                        "Collection ordering failed, as source {0} and destination {1} can not be same",
                        srcPath, destPath);
                log.error(message);
